...
func fetchStock(ticker string, ch chan<- StockResult, wg *sync.WaitGroup) {
	defer wg.Done()
	...
	if err != nil {
		ch <- StockResult{Ticker: ticker, Error: fmt.Errorf("error fetching data for %s: %s", ticker, err)}
		return
	}

	if err := json.NewDecoder(resp.Body).Decode(&data); err != nil {
		ch <- StockResult{Ticker: ticker, Error: fmt.Errorf("error decoding data for %s: %s", ticker, err)}
		return
	}

	if len(data.Results) == 0 {
		ch <- StockResult{Ticker: ticker, Error: fmt.Errorf("no data found for %s", ticker)}
		return
	}

	price := data.Results[0].Price
	ch <- StockResult{Ticker: ticker, Price: price, Error: nil}
}

func main() {
	startNow := time.Now()
	defer func() { fmt.Printf("This operation took: %s", time.Since(startNow)) }()
